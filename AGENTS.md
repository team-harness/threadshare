...
2. Run the full verification above plus `npm run test:insights-engine` and `npm pack --dry-run --ignore-scripts --json` with Node 22.22.3 and npm 12.0.2. Confirm the exact 92-file source-root allowlist, the 352 KiB compressed root limit, and each platform package's exact four-file allowlist from `scripts/verify-release.mjs`.
