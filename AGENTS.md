...
2. Run the full verification above plus `npm pack --dry-run --ignore-scripts --json` with Node 22.22.3 and npm 12.0.2. Confirm the exact 18-file allowlist defined by `EXPECTED_PACKAGE_FILES` in `scripts/verify-release.mjs` and record its integrity.
