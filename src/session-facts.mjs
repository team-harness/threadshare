...
function assertWellFormedUnicode(value) {
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (code >= 0xd800 && code <= 0xdbff) {
      ...
      if (next < 0xdc00 || next > 0xdfff) throw new TypeError("strings must not contain unpaired surrogates");
      index += 1;
...
function canonicalValue(value, ancestors) {
  ...
  try {
    if (Array.isArray(value)) {
      const values = [];
      for (let index = 0; index < value.length; index += 1) values.push(canonicalValue(value[index], ancestors));
      return `[${values.join(",")}]`;
    }
    ...
    return `{${entries.map(([key, item]) => `${JSON.stringify(key)}:${canonicalValue(item, ancestors)}`).join(",")}}`;
  } finally {
    ancestors.delete(value);
  }
}

/** RFC 8785-compatible canonical JSON for the Fact Model's restricted value domain. */
