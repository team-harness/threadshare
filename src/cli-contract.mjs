...
export const OPTION_DEFINITIONS = Object.freeze({
  ...
  runner: {
    ...
    description: "Launch a separate Claude Code or Codex CLI for an optional batch workflow.",
  },
  "runner-model": {
    type: "value",
    placeholder: "<model>",
    description: "Bind a new Codex extraction preview to this exact model.",
  },
  "runner-endpoint": {
    type: "value",
    placeholder: "<https-url>",
    description: "Bind a new Codex extraction preview to this exact HTTPS endpoint.",
  },
  "approve-plan": {
    type: "value",
    placeholder: "<digest>",
  ...
  since: {
    ...
    description: "Start a Team Memory retrospective at this canonical UTC timestamp.",
  },
  until: {
    type: "value",
    placeholder: "<utc-timestamp>",
    description: "End a Team Memory retrospective at this canonical UTC timestamp.",
  },
  providers: {
    type: "value",
    placeholder: "<claude,codex>",
    description: "Filter Team Memory extraction to comma-separated session providers.",
  },
  "session-keys": {
    type: "value",
    placeholder: "<hex64,...>",
    description: "Filter Team Memory extraction to comma-separated opaque session keys.",
  },
  "tool-capability-keys": {
    type: "value",
    placeholder: "<hex64,...>",
    description: "Filter Team Memory extraction to comma-separated Tool capability keys.",
  },
  "skill-capability-keys": {
    type: "value",
    placeholder: "<hex64,...>",
    description: "Filter Team Memory extraction to comma-separated Skill capability keys.",
  },
  "result-evidence": {
    type: "value",
    placeholder: "<state,...>",
    description: "Filter Team Memory extraction by comma-separated result-evidence states.",
  },
  "capability-terminal-states": {
    type: "value",
    placeholder: "<state,...>",
    description: "Filter Team Memory extraction by comma-separated capability terminal states.",
  },
  "if-due": {
    type: "boolean",
    description: "Create a consolidation plan only when at least 20 approved L1 entries changed.",
  },
  full: {
    type: "boolean",
    description: "Replay all current approved L1 entries during consolidation.",
  },
  kind: {
    type: "value",
    placeholder: "<entry|skill|consolidation>",
    description: "Select the Team Memory candidate kind to review.",
  },
  plan: {
    type: "value",
    placeholder: "<plan-id>",
  ...
  provider: {
    ...
    description: "Assemble team memory into this provider's context file: claude or codex.",
  },
  offset: {
    type: "value",
    placeholder: "<n>",
...
export const COMMAND_SPECS = Object.freeze({
  ...
  memory: command({
    ...
    arguments: [
      argument(
        "action",
        `<${MEMORY_CLI_ACTIONS.join("|")}>`,
        "Team Memory lifecycle action.",
      ),
      argument("path", "[path...]", "Only for lint: entry or Skill files to check; defaults to the entries directory.", true),
    ],
    options: [
      "approve-manifest",
      "approve-plan",
    ...
    optionDetails: {
      ...
      limit: "For recall or extract. Number of bounded chunks to return, plan, or deliver; 1 through 8, default 1.",
      since: "For a new recall or extract. Required with --until when using human-readable parameters.",
      until: "For a new recall or extract. Required with --since; both timestamps use canonical UTC with milliseconds.",
      query: "For a parameterized recall or extract. Optional full-text query over indexed Turns.",
      providers: "For a parameterized recall or extract. Comma-separated claude/codex values.",
      "session-keys": "For a parameterized recall or extract. Comma-separated opaque session keys.",
      "tool-capability-keys": "For a parameterized recall or extract. Comma-separated Tool capability keys.",
      "skill-capability-keys": "For a parameterized recall or extract. Comma-separated Skill capability keys.",
      "result-evidence": "For a parameterized recall or extract. Comma-separated provider-completed, abandoned, or unknown values.",
      "capability-terminal-states": "For a parameterized recall or extract. Requires a Tool or Skill capability key filter.",
      request: "For recall/extract advanced filters, or the exact SkillCandidate, CandidateDraftBatch, AdjudicationResult, ConsolidationPatch, or PrepareRequest consumed by stage/prepare. Read JSON from a file or stdin; recall/extract cannot mix it with human-readable filters.",
      plan: "Only for promote. The promotion plan id produced by review.",
      provider: "Only for assemble. The provider context file to update: claude or codex.",
      kind: "Only for review. Values: entry (default), skill, or consolidation.",
      "if-due": "For synthesize or consolidate. Continue only when at least 20 approved L1 entries are new or changed.",
      full: "For synthesize or consolidate. Ignore the successful baseline and replay all approved L1 entries.",
    },
    defaults: [
      "--format text",
    ...
    output: [
      ...
      "recall: relevant existing Skills, digest-bound scenes/doctrine/approved entries, then complete bounded Turn chunks plus evidence ids; synthesize: approved L1 plus current scene/doctrine state.",
      "stage: SkillCandidate is quarantined with evidence; CandidateDraftBatch returns an AdjudicationTask; AdjudicationResult quarantines retained candidates. review returns one exact approval preview; prepare validates its digest and creates a CAS-bound plan; promote applies it.",
      "extract: without approval, pending extraction plans; after extraction, separate pending adjudication plans.",
      "consolidate: without approval, one pending L2/L3 plan; after approval, a no-op baseline or quarantined patch.",
      "assemble: regenerates the provider memory block; reverify-runner refreshes the conformance cache.",
    ],
    constraints: [
      "init, status, extract, consolidate, review, promote, and assemble require a non-bare Git worktree (owner resolution).",
      "extract never spawns a runner without a matching --approve-plan or --approve-manifest digest.",
      "consolidate never spawns a runner without a matching --approve-plan digest.",
      ...
      "--since/--until or --request creates a new preview and cannot be combined with an approval digest.",
      "Human-readable request parameters cannot be combined with --request.",
      "recall and synthesize return their bounded source directly to the calling Agent and never launch another runner.",
      "Skill synthesis reads skillContext, then memoryContext, then historical Turns; SkillCandidate must echo memoryContext.bindingDigest as memoryContextDigest.",
      "stage and prepare consume exact JSON from --request; an Agent should pass it through stdin instead of asking a person to author a file.",
      "Process recalled sources one at a time with the default --limit 1 unless the Agent context can hold every requested chunk.",
      "Codex preview binds an explicit --runner-model and --runner-endpoint; approval reuses the private stored profile and cannot override either.",
      "promote writes only into the worktree; it never stages, commits, or pushes.",
      "A new extraction preview requires --since plus --until, or the advanced --request input, and searches only that explicit time window and filters.",
    ...
    examples: [
      ...
      "threadshare memory recall --since <utc> --until <utc> --query <text> --providers claude,codex --format json",
      "printf '<CandidateDraftBatch@v1>' | threadshare memory stage --request - --format json",
      "printf '<SkillCandidate@v1>' | threadshare memory stage --request - --format json",
      "printf '<AdjudicationResult@v1>' | threadshare memory stage --request - --format json",
      ...
      "threadshare memory extract --runner claude --since <utc> --until <utc> --query <text> --providers claude,codex",
      "threadshare memory extract --runner codex --runner-model <model> --runner-endpoint <https-url> --since <utc> --until <utc>",
      ...
      "threadshare memory consolidate --runner codex --runner-model <model> --runner-endpoint <https-url>",
      ...
      "threadshare memory reverify-runner --runner codex --runner-model <model> --runner-endpoint <https-url>",
    ],
    agentNotes: [
      ...
      "In an existing Codex or Claude conversation, prefer recall/synthesize and analyze the returned source yourself; do not specify --runner.",
      "For transcript claims, cite the evidence id from chunk.turnEvidence or the matching <<past-turn>> marker; never infer ev-* ids from their order.",
      "For a Skill, inspect existing Skills and digest-bound scenes/doctrine/approved entries before the Turn source; if either context is truncated, narrow the recall query.",
      "Show proposed wording to the user before stage. SkillCandidate is evidence-bound and goes to quarantine; CandidateDraftBatch returns an AdjudicationTask, which must be compared before an exact store/skip/update/merge result is staged.",
      "After adjudication, show review.approval once. After user confirmation pass approval.prepareRequest verbatim to prepare; if it echoes the same approvalDigest, promote without another prompt. Any changed preview requires a new confirmation.",
    ...
    security: [
      "Agent-native recall intentionally returns bounded transcript content to the current trusted Agent; Threadshare does not authenticate the human separately from that Agent.",
    ...
    environment: [
      "THREADSHARE_INSIGHTS_HOME overrides derived state; THREADSHARE_MEMORY_RUNNER_BIN overrides the runner binary and THREADSHARE_MEMORY_CODEX_BIN can target Codex specifically.",
      "THREADSHARE_MEMORY_RUNNER_MODEL and THREADSHARE_MEMORY_RUNNER_ENDPOINT may supply the exact Codex settings when the paired CLI options are omitted.",
    ],
  }),
  messages: command({
...
const INSIGHTS_ACTION_HELP = Object.freeze({
  ...
  recipe: [
    ...
    "  optional comparisonWindow, filters, limit (normally 1..50), allowDegraded (default false)",
    "  extraction-candidates@1 requires exact turnKeys, allows limit 1..66, and rejects degraded reads.",
