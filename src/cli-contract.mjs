...
export const OPTION_DEFINITIONS = Object.freeze({
  ...
  kind: {
    type: "value",
    placeholder: "<entry|consolidation>",
...
export const COMMAND_SPECS = Object.freeze({
  ...
  memory: command({
    ...
    optionDetails: {
      ...
      request: "For recall/extract advanced filters, or the exact CandidateDraftBatch, AdjudicationResult, ConsolidationPatch, or PrepareRequest consumed by stage/prepare. Read JSON from a file or stdin; recall/extract cannot mix it with human-readable filters.",
      ...
      kind: "Only for review. Values: entry (default) or consolidation.",
    ...
    output: [
      ...
      "recall: complete bounded Turn chunks plus evidence ids for the current Agent; synthesize: approved L1 plus current scene/doctrine state.",
      "stage: CandidateDraftBatch returns an AdjudicationTask; AdjudicationResult quarantines retained candidates. review returns exact digests; prepare creates a CAS-bound plan; promote applies it.",
    ...
    agentNotes: [
      ...
      "Show proposed wording to the user before stage. CandidateDraftBatch returns an AdjudicationTask; compare its pool and stage an exact store/skip/update/merge AdjudicationResult before review.",
